...
export const drawGroundDetails = (
  ...
  /** Рисуемые клетки; без диапазона — вся карта. */
  range?: CellRange,
) => {
  ctx.save();
  ...
  forEachCellIn(grid, range, (cell, x, y) => {
    ...
    const nearForest = neighbors.some(neighbor => neighbor?.type === 'forest');
    const nearRock = neighbors.some(neighbor => neighbor?.type === 'mountain');
