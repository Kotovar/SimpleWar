...
склеивается, объём ограничен, журнал очищается при новой партии. Обычному
журналу участника доступны только его действия (`getVisibleRecords`), полный —
для отладки. Ошибка старта идёт тем же путём.
