...
| `src/entities`              | Zustand-хранилища карты, настроек, юнитов, зданий и экономики      |
...
В `package.json` нет команды тестов. Для изменений документации достаточно
проверить ссылки и форматирование только затронутых Markdown-файлов.
Не запускай глобальный `format` ради одного файла.
